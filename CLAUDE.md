...
└── shared/
    ├── domain/                      # app-error, base-entity, error subclasses, jwt-service, password-hasher
    ...
    └── utils/
        ├── pagination.ts            # paginationQuery Zod schema, idParams, helpers
        ├── response.ts              # responsePlugin (jsonOk, jsonOkNoData, jsonPaginated, jsonError)
...
```
Use response helpers from context: `jsonOk(data)`, `jsonOk(data, message)`, `jsonOkNoData(message)`, `jsonPaginated(data, total, page, pageSize)`, `jsonError(code, message)`. NEVER return raw objects.
