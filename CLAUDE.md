...
railway.json                         # Config as code for the api service
railway.jobs.json                    # Config as code for the cron service
...
- Two services share this repo and the same commit, each pointing at its own config-as-code file: `railway.json` (api) and `railway.jobs.json` (cron). Keep them in sync when a build input changes, including `watchPatterns`.
