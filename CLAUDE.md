...
scripts/
├── deploy-prepare.ts                # Railway pre-deploy: migrate, then seed if SEED_ON_DEPLOY
└── run-scheduled-jobs.ts            # entrypoint of the Railway cron service (posts /api/v1/jobs/*)
...
railpack.json                        # Railpack builder config (pins Bun)
.railway/railway.ts                  # Railway Infrastructure as Code: db, both services, variables
.github/workflows/ci.yml             # lint + type-check + tests; Railway gates deploys on it
RAILWAY.md                           # Deployment runbook + dashboard checklist
```
...
| `JOBS_API_URL` | falls back to `API_URL` | Base url the cron service posts the job endpoints to. Read by `scripts/run-scheduled-jobs.ts` only, NOT by `env.ts` |
| `SEED_ON_DEPLOY` | unset | `"true"` re-runs the idempotent seed on every deploy. Read by `scripts/deploy-prepare.ts` only, NOT by `env.ts` |
...
bun run build         # Prisma generate — the build step Railpack runs on Railway
...
bun run jobs:run      # Run the scheduled jobs against JOBS_API_URL (what Railway cron runs)
```
...
- `scripts/run-scheduled-jobs.ts` is the cron entrypoint and MUST stay standalone: it reads only `JOBS_API_URL`/`API_URL` and `JOB_SECRET`, never `@/config/env`. Importing the app env would force the cron service to carry a database url, a Resend key and storage credentials it has no use for.
- That script MUST call `process.exit()`. Railway skips the next scheduled run while the previous one is still `Active`, so a container that lingers silently stops the notifications.
- Deploys are gated on `.github/workflows/ci.yml` through Railway's **Wait for CI**. A workflow rename breaks the gate — update the dashboard with it.
- `STORAGE_DRIVER=local` is NOT viable on Railway without a volume: the filesystem is wiped on every deploy. Production uses `s3`.
