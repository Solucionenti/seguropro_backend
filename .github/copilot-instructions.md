...
- `.railway/railway.ts` MUST keep its `export const partial` declaration. The Railway project also holds `seguropro_front` from another repository; without the partial, omission-means-deletion would destroy it. Never rename the partial — that orphans everything it owns.
- Resource names in the IaC file MUST match the live ones (`Postgres`, `seguropro_backend`, `seguropro_jobs`). A mismatch does not adopt: it creates a new service and DELETES the real one. Always read `railway config plan` and treat any `destroy` line as a stop sign.
